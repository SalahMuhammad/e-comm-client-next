...
function RepositoryPermitButton({ id, type, permitValue, width = '120px', isInView = false }) {
    ...
    // Modern button style for view page
    if (isInView) {
        return (
            <button
                ...
                style={{
                    backgroundColor: permit ? '#16a34a' : '#3f3f46',
                    opacity: isLoading ? 0.7 : 1,
                    minWidth: '150px'
                }}
                className="inline-flex cursor-pointer items-center justify-center gap-2 px-4 py-2 text-white rounded-lg transition-all duration-200 text-sm font-medium shadow-sm hover:shadow-md disabled:cursor-not-allowed"
            >
                {isLoading && (
