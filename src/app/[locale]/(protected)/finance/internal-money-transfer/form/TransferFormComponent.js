...
function TransferForm({ initialData }) {
    ...
    return (
        <GenericFormShell
            ...
            obj={initialData}
            t={t}
            redirectPath={`/finance/internal-money-transfer/view/${state.data?.hashed_id || initialData?.hashed_id}`}
            isModal={false}
            customTitle={`${initialData?.id ? t("finance.fields.update") : t("finance.fields.transaction")} ${t("MainMenu.finance.subLabels.internalTransfer")} ${initialData?.hashed_id ? "#" + initialData.hashed_id : ""}`.trim()}
            showIdField={false}
        >
